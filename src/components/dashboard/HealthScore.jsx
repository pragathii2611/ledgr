...
function calculateHealthScore(transactions) {
  ...
  const savingsScore = Math.min(savingsRate * 200, 40);
  const diversityScore = Math.min(incomeTypes * 10, 20);
  const concentrationScore = Math.max(0, (1 - topCatPct) * 20);
  const budgetScore = budgetAdherence * 20;

  return Math.round(
    savingsScore + diversityScore + concentrationScore + budgetScore
  );
}

function getScoreLabel(score) {
  if (score >= 80) return { label: "Excellent", color: "#4F46E5" };
...
export default function HealthScore({ transactions, isLoading }) {
  ...
  const breakdown = [
    {
      label: "Savings Rate",
      value: Math.min(savingsRate * 200, 40),
      max: 40,
    },
    {
      label: "Income Sources",
      value: Math.min(incomeTypes * 10, 20),
      max: 20,
    },
    {
      label: "Spend Balance",
      value: Math.max(0, (1 - topCatPct) * 20),
      max: 20,
    },
    {
      label: "Budget Control",
      value: budgetAdherence * 20,
      max: 20,
    },
  ];

  return (
    <div className="bg-white dark:bg-[#242422] border border-[#EBEBE8] dark:border-[#2E2E2C] rounded-card shadow-card p-6 flex flex-col">
      <div className="mb-5">
          ...
          {breakdown.map((b) => (
            <div key={b.label}>
              <div className="flex items-center justify-between mb-1.5">
                <span className="text-xs text-[#6B6B68] dark:text-[#8C8C88]">
                  {b.label}
                </span>
      ...
      {/* bottom summary */}
      <div className="mt-5 pt-4 border-t border-[#EBEBE8] dark:border-[#2E2E2C]">
        <p className="text-xs text-[#A8A8A5]">
          Score based on {transactions.length} transactions across{" "}
